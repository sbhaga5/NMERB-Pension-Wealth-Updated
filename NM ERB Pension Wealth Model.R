...
GetNormalCost <- function(HiringAge, StartingSalary){
  ...
  SurvivalMale <- left_join(SurvivalMale,MaleMortality,by = 'Age') %>% group_by(Age) %>% 
    ...
    mutate(MPValue2 = cumprod(1-lag(`2036`,2,default = 0)), YOS = Age - HiringAge,
  ...
  AdjustedMale <- SurvivalMale %>% filter(Age - (Years - BaseYear) == HiringAge, Years >= BaseYear)
  AdjustedFemale <- SurvivalFemale %>% filter(Age - (Years - BaseYear) == HiringAge, Years >= BaseYear)
...
for(i in 1:nrow(SalaryHeadcountData)){
  SalaryHeadcountData$NormalCost[i] <- GetNormalCost(SalaryHeadcountData$Hiring_Age[i], SalaryHeadcountData$Starting_Salary[i])
}
